...
# weighted median: 加权中位数法。
# Mode based: 模式估计法。
# MR.Mix: 混合模型法。
# MR.RAPS: 基于Huber损失函数的MR-RAPS。
# mrcML.DP 和 CDcML.DP.S: 自定义的两种估计方法。
# CD.Ratio 和 CD.Egger: CD方法中的比率法和Egger法。
# 结果包含估计值、标准误差、置信区间和p值。

Method_UVMR <- function(bx, bxse, by, byse,N,method_name){
  # bx 和 bxse：暴露变量的效应量及其标准误差。
  # by 和 byse：结果变量的效应量及其标准误差。
  # N：样本量。
  result <- NULL
  ...
  for(i in 1:D){
    for(j in 1:D){
      ...
      res <- Method_UVMR(bx=data_sum_beta[llp,i], 
                         ...
                         N,method_name)
      amatt_pleio[i,j] <- res$direction    # 返回一个对称矩阵amatt_pleio，值为0或1，表示是否存在显著因果关系。
    }
  }
