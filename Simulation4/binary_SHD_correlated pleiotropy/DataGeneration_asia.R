...
DataGeneration_asia <- function(N,each_g,gl,gu,dlB,duB,D_beta,prob_corp){
  ...
  G_beta[(D*each_g+1):total_g,] <- runif(D*each_g,gl,gu)
  ...
  if(sum(!lllo)!=0){
    ...
    for(oe in 1:length(aftD)){
      ...
      if(length(parennode)==1){
        pp <- data_ind[,parennode] * D_beta[parennode,aftD[oe]] + G %*% G_beta[,aftD[oe]] + U + rnorm(N,0,1)
      }else{
        pp <- data_ind[,parennode] %*% D_beta[parennode,aftD[oe]] + G %*% G_beta[,aftD[oe]] +  U + rnorm(N,0,1)
      }
      pp1 <- exp(pp)/(1+exp(pp))
  ...
  for(k in 1:total_g){
    for(d1 in 1:D){
      fit <- glm(data_ind[,d1]~G[,k],family=binomial(link='logit'))
