...
Comp <- function(x,N,D_beta,each_g,gl,gu,dlB,duB,prob_corp,D_beta_M){
  ...
  fdata <- DataGeneration_asia(N,each_g,gl,gu,dlB,duB,D_beta,prob_corp)
  R <- fdata$D_beta
  ...
  #################### MRSL ####################################################
  # MRSL(Mendelian Randomization using Structural Learning)：执行不同方法，如加权中位数法、模式法等。
  #remove collier
  ...
  #################### MRPC ####################################################
  # MRPC (MR Pathway Construction)：基于 disCItest 或 gaussCItest。#############
  locdd <- NULL
  for(dd in 1:D){
  ...
  # cGAUGE：分别使用 IVW、Egger 和 MR-PRESSO 方法。#############################
  #################### cGAUGE ivw ##############################################
  ptm <- Sys.time()
